/*
	Нахождение предыдущих состояний "в лоб" за O(N^2)
*/
...
int main()
{
	...
	for(i=1;i<n;i++)
	{
		...
		{
			for(p=i-1;p>=0 && nearest[p][0]==k && (nearest[p][1]==k || nearest[p][1]==-1); p--);
			for(q=1;p>=0 && q>=0 && nearest[p][q]==k; q--);
			f[i][0] = getf(p,q) + i-1-p;
		}else
