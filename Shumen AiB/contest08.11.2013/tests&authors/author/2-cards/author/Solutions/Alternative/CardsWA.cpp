...
int recurse(int idx) {
	...
	for (int i = 0; i < 5; i++) {
		...
		int at = i;
		while (at > 0 && hand[at].idx < hand[at - 1].idx)
			{swap(hand[at], hand[at - 1]); at--;}
		while (at < 4 && hand[at].idx > hand[at + 1].idx)
			{swap(hand[at], hand[at + 1]); at++;}
		...
		cntSuit[hand[at].suit]--; cntRank[hand[at].rank]--;
		memcpy(hand, save, sizeof(hand));
		cntSuit[save[i].suit]++; cntRank[save[i].rank]++;
	}
