...
void update(int i, int dir) {
  ...
  if (dir == 1) {
    ...
    start = people[i].size() - 1;
    end = -1;
  }
  for (int j = start; j != end ; j += dir) {
    const string& s = people[i][j];
    int lcp_last = 0;
    if (j > 0) lcp_last = lcp(s, people[i][j - 1]);
    ...
    while ((dir == 1 && ptr + 1 < people[i - 1].size() && people[i - 1][ptr + dir] <= s) ||
       (dir == -1 && ptr - 1 >= 0 && people[i - 1][ptr + dir] >= s)) {
      ptr += dir;
...
bool solve() {
  ...
  for (int i = 1; i < LEVEL; ++i) {
    ...
    ans[i].resize(k);
    parent[i].resize(k);
  forn(j, people[i].size()) ans[i][j] = INF;
    update(i, 1);
    update(i, -1);
  }
  return true;
