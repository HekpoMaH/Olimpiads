...
public class deer_sm_wrong {
	...
	void solve() {
		...
		double ans = S + STR + length * length * Math.PI / 2;
		if (DEBUG)
			System.err.println(p[pos] + " -- " + goat);
		if (p[pos].equals(goat)) {
			Point vec1 = p[pos].subtract(p[(pos + n - 1) % n]);
			Point vec2 = p[pos].subtract(p[(pos + n + 1) % n]);
			if (DEBUG)
				System.err.println(vec1 + " " + vec2);
			ans += length * length * angle(vec1, vec2) / 2;
		}
		if (DEBUG)
