...
bool calc(vector<StoreStep>& to,
            vector<StoreStep>& back,
            int resi,
            int resj,
            double& ans
            )
{
	...
	if (resi == (int)to.size() || resj == (int)back.size()) {
		return false;
	}

    StoreStep t = to[resi];
    StoreStep b = back[resj];
    circle f(t.p.x, t.p.y, t.len);
    circle s(b.p.x, b.p.y, b.len);
    vector<point> res, nres;
    int c = cross(f, s, res);
    if (c != -1 && c != 0 && resi != 0 && resj != 0) {
        forv(q, res) {
...
void solve() {
    ...
    ans = 0;
    calc(to, back, to.size(), back.size(), ans);
    forv(i, to) {
        forv(j, back) {
            double cur;
            if (calc(to, back, i, j, cur)) {
                ans = min(ans, cur);
            }
        }
    }
