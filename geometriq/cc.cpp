...
int main()
{
    ...
    for(int i=1;i<=n;i++)
    {
        ...
        if(a[i].x==p0.x&&a[i].y==p0.y)idx=i;
    }
    for(int i=1;i<=n;i++)
    {
        a[i+n]=a[i];
    ...
    //exit(0);
    ob.push_back(a[idx]);ob.push_back(a[idx+1]);
    for(int i=idx+2;i<=n+idx-1;i++)
    {
        cout<<"--->"<<a[i].x<<" "<<a[i].y<<endl;
        if(ori(ob[ob.size()-2],ob[ob.size()-1],a[i])>=0)ob.push_back(a[i]);
        else{while(ori(ob[ob.size()-2],ob[ob.size()-1],a[i])<0&&ob.size()>2)ob.pop_back();ob.push_back(a[i]);}
        cout<<"-------------"<<endl;
