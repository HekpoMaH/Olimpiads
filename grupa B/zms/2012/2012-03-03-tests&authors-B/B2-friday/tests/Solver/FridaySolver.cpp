...
void solve(int testNum)
{
	...
	sprintf(inpFile, "Friday.%02d.in", testNum); in = fopen(inpFile, "rt");
	sprintf(outFile, "Friday.%02d.sol", testNum); out = fopen(outFile, "wt");
...
int main(void)
{
	...
	for (int test = 0; test <= numTests; test++)
	{
		...
		fprintf(stderr, "Currently solving testcase %d... ", test);
		solve(test);
